    ...
    if (!prevStepBack)
    {
      ...
      for (int i=0; i<1024; i++)
      {
        if (xCntArr[i] > 0)
        {
          ...
          server.print(buf);//1536 hz
        }
      }
      server.print("END FRAME: ");
      server.print(millis() - frameStartMs, DEC);
      server.print(" ms, ");
      server.print(framePoints, DEC);
      server.println(" points");
      zeroXArr();
    }
    prevStepBack = true; 
    framePoints = 0;
