...
#define LED1_PIN A1
#define LED2_PIN A2
#define POWER_SWITCH_PIN A3
#define MODEM_ENABLE_PIN A4
#define POWER_ENABLE_PIN A5
#define SIM_DRAWER_PIN 9
...
#define WATCHDOG_TRIGGER_PERIOD 1000 //ms - of generated pulses signal
#define WATCHDOG_TIMEOUT 30000 //ms if WD activated and no serial activity for timeout - stop pulses

#define POWER_BUTTON_TIMEOUT 3000

boolean serialWatchDogArmed = false;
long watchDogLastTriggered = 3000; //3 sec for boot
long serialLastRecieved = 0;
...
boolean prevPowerButton = true;
long buttonStateChanged = 0l;
...
void setup() {     
  ...
  pinMode(SIM_DRAWER_PIN, INPUT_PULLUP);
 
  digitalWrite(POWER_SWITCH_PIN, HIGH);
  digitalWrite(GAS_SENSOR_PIN, HIGH);
  digitalWrite(SIM_DRAWER_PIN, HIGH);
  ...
  digitalWrite(MODEM_ENABLE_PIN, false);
...
void loop() {
   ...
   boolean powerButtonState =  digitalRead(POWER_SWITCH_PIN);
   if (powerButtonState != prevPowerButton) 
   {
     buttonStateChanged = millis();
     prevPowerButton = powerButtonState;
   }
   
   if (!powerButtonState && millis() - buttonStateChanged > POWER_BUTTON_TIMEOUT && !shutdownSent) 
   {
     responseln("POWER_SWITCH: false");
     shutdownSent = true;
   } 
   ...
   /* if (powerOffScheduledTime != 0 && millis() > powerOffScheduledTime && digitalRead(POWER_ENABLE_PIN)) 
    {
        responseln("shutting down power");
         digitalWrite(POWER_ENABLE_PIN, false);
    }*/
    
    if (!serialWatchDogArmed || millis() - serialLastRecieved < WATCHDOG_TIMEOUT) {
      if (millis() - watchDogLastTriggered > WATCHDOG_TRIGGER_PERIOD) {
        watchDogLastTriggered = millis();
        if (powerOffScheduledTime == 0 || millis() < powerOffScheduledTime) {
          if (digitalRead(POWER_SWITCH_PIN))
            digitalWrite(POWER_ENABLE_PIN, !digitalRead(POWER_ENABLE_PIN));
        }
      }
    }
...
void parseReceivedText()
{
  ...
  if (textBuffer.substring(0,2) == "GS") 
  {
    ...
    response("POWER_SWITCH: ");
    
    if (!digitalRead(POWER_SWITCH_PIN) && millis() - buttonStateChanged > POWER_BUTTON_TIMEOUT) 
      responseln("false");
    else
       responseln("true");
    ...
    response("SIM_DRAWER: ");
    responseln(digitalRead(SIM_DRAWER_PIN)?"true":"false");
    
    response("MODEM_ENABLED: ");
    responseln(digitalRead(MODEM_ENABLE_PIN)?"false":"true");
    
    response("POWER_ENABLED: ");
    responseln(digitalRead(POWER_ENABLE_PIN)?"true":"false");
    
    response("CHARGE_ENABLED: ");
    responseln(digitalRead(CHARGE_ENABLE_PIN)?"true":"false");
    
    response("SERIAL_WATCHDOG: ");
    responseln(serialWatchDogArmed?"true":"false");
  ...
  else if (textBuffer.substring(0,6) == "POWER=") 
  {
    textBuffer = textBuffer.substring(6);
    if (textBuffer.substring(0,2) == "ON")
  ...
  else if (textBuffer.substring(0,9) == "POWEROFF=") {
    textBuffer = textBuffer.substring(9);
    long timeout = parseLongDigits();
    if (timeout > 0 && timeout < 200) {
      powerOffScheduledTime = millis() + (timeout * 1000);
      printOK();
    }
    else
    {
  ...
  else if (textBuffer.substring(0,4) == "SWD=") 
  {
    textBuffer = textBuffer.substring(4);
    if (textBuffer.substring(0,2) == "ON")
      serialWatchDogArmed = true;
    else if (textBuffer.substring(0,3) == "OFF")
      serialWatchDogArmed = false;
    else 
    {
      printErrorMessage();
      return;
