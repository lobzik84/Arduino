...
#define MICRON1_SERIAL_BAUD_RATE 1200
#define MICRON2_SERIAL_BAUD_RATE 1200

#define ADC_CYCLES_PER_CHANNEL 2000
...
SerialLinMicron micron1;
SerialLinMicron micron2;
...
void setup()
{
 ...
 // Open serial communications and wait for port to open:
  ...
  //ADMUX |= 0x20;//   // set A8-15 analog input pin
 // ADCSRB |= 0x08; // set A8-15 analog input pin
  ADMUX |= (1 << REFS1);  // set reference voltage 1.1V
...
//  micron1.serialBuffer = {0,0,0,0,0,0};
  micron1.bufferIndex = 0;
  micron1.calculatedCrc = 0;;
  micron1.receivedCrc = 0;
  micron1.freq = 0;
  micron1.headerReceived = false;
  

//  micron2.serialBuffer = {0,0,0,0,0,0};
  micron2.bufferIndex = 0;
  micron2.calculatedCrc = 0;;
  micron2.receivedCrc = 0;
  micron2.freq = 0;
  micron2.headerReceived = false;
...
ISR(ADC_vect)
{
  ...
  if (bufferIndex == ADC_CYCLES_PER_CHANNEL) {
    ...
    adcBuffer = 0;
    channelIndex++;
    if (channelIndex >=16) channelIndex = 0;
    if (channelIndex >= 8) {
      //add ADCSRB bit 3 & ADMUX bit 5 for A8-A15
      ADMUX &= 0xF8;
      ADMUX |= 0x20;//set ADMUX bit 5 for A8-A15
      ADCSRB |= 0x08;// set ADCSRB bit 3 for A8-A15
    } else {
      ADMUX &= 0xD8; //clear bit 5 for A0-A7
      ADCSRB &= 0xF7; //clear bit 3 for A0-A7
    }
    ADMUX |= channelIndex & 0x07; //add ADCSRB bit 3 & ADMUX bit 5 for A8-A15
    bufferIndex = 0;
  }
}
...
void loop()
{
  ...
  if (adcReading != MAX_ULONG) {
    responseln ("ADC" + String(channelReading) + ": " + String(adcReading/ADC_CYCLES_PER_CHANNEL)); 
  ...
  if (Serial1.available())
    readSerial(&Serial1, 1, micron1);
  if (Serial2.available())
    readSerial(&Serial2, 2, micron2);

}

void readSerial(Stream *str, int portNum, SerialLinMicron micron)
{
  char c;
  while (str->available())
  {
    ...
    responseln("Serial" + String(portNum) + ":0x" + String(buffer));
    
    char sixZeroes[6] = {0,0,0,0,0,0};
    if (c == 0xAA && ( memcmp( (const void *)micron.serialBuffer, (const void *)sixZeroes, 6) == 0)) {
      responseln("Micron" + String(portNum)  + " header recieved");
      micron.bufferIndex = 0;
      micron.headerReceived = true;
    }
    else {
      micron.serialBuffer[micron.bufferIndex] = c;
      micron.bufferIndex++;
      if (micron.bufferIndex >=6 ) {
        micron.bufferIndex = 0;
      }
    }

    if (micron.headerReceived && micron.bufferIndex == 4) {
      micron.headerReceived = false;
      micron.bufferIndex = 0;
      
      response("Micron" + String(portNum)  + " data received: ");
      
      for (int i = 0; i<5; i++) {
        char buffer[3];
        sprintf(buffer, "%02x", micron.serialBuffer[i]);
        responseln(" 0x" + String(buffer));
      }
      
      responseln("");

      micron.calculatedCrc = micron.serialBuffer[4];//TODO CRC check
      micron.receivedCrc = micron.serialBuffer[4];

      if (micron.calculatedCrc == micron.receivedCrc) {
        tmp.b[0] = micron.serialBuffer[0];
        tmp.b[1] = micron.serialBuffer[1];
        tmp.b[2] = micron.serialBuffer[3];
        tmp.b[3] = 0;
        micron.freq = tmp.l;
        response("FREQ" + String(portNum)  + ": " + String(micron.freq));
      } else {
        responseln("Micron" + String(portNum)  + " CRC error. Calculated: " + String(micron.calculatedCrc) + " Received: " + String(micron.receivedCrc)  );
      }
