...
//  micron1.serialBuffer = {0,0,0,0,0,0};
  ...
  micron.calculatedCrc = 0;;
  micron.receivedCrc = 0;
  micron.freq = 0;
...
ISR(ADC_vect)
{
  ...
  if (bufferIndex == ADC_CYCLES_PER_CHANNEL) {
    ...
    if (channelIndex >= 8) {
      ...
      ADCSRB = 0; //clear bit 3 for A0-A7
    }
    ADMUX = (channelIndex & 0x07) | 0x80; //3 channel bits, add REFS1 bit, other is 0
    bufferIndex = 0;
  }
}
...
void readSerial(Stream *str, int portNum)//, SerialLinMicron micron)
{
  ...
  while (str->available())
  {
    ...
    if (micron.headerReceived && micron.bufferIndex == 4) {
      ...
      /*responseln("Micron" + String(portNum)  + " data received: ");
...
*/
      ...
      if (micron.calculatedCrc == micron.receivedCrc) {
        ...
        tmp.b[2] = micron.serialBuffer[2];
        tmp.b[3] = micron.serialBuffer[3];
        micron.freq = tmp.l;
        responseln("FREQ" + String(portNum)  + ": " + String(micron.freq));
      } else {
