...
#define MOTOR_CURRENT_TIMEOUT 300
#define MIN_US_NOACC 100
#define PULSES_ACC_CNT 16

#define ENX A0 // stepper motor enable , active low pin
#define ENY A1
#define X_DIR 5 // X -axis stepper motor direction control pin
#define Y_DIR 6 // y -axis stepper motor direction control pin
#define X_STP 2 // x -axis stepper control pin
#define Y_STP 3 // y -axis stepper control pin
...
void loop()
{
  ...
  if (millis() - lastStep > MOTOR_CURRENT_TIMEOUT) {
    digitalWrite (ENX, HIGH);
    digitalWrite (ENY, HIGH);
  }
}

 void step(boolean dir, byte dirPin, byte stepperPin, long steps, byte enablePin)
{
  int delayUs = stepUs;
  if (delayUs < MIN_US_NOACC) delayUs = MIN_US_NOACC;
  ...
  for (long i = 0; i <steps; i++) 
  {
    ...
    if (steps-i > MIN_US_NOACC*PULSES_ACC_CNT && stepUs < delayUs && i%PULSES_ACC_CNT==0  ) {
      delayUs--;
    }
    delayMicroseconds (delayUs);
    digitalWrite (stepperPin, LOW);
    if (steps-i < MIN_US_NOACC*PULSES_ACC_CNT && delayUs < MIN_US_NOACC && i%PULSES_ACC_CNT==0  ) {
      delayUs++;
    }
    delayMicroseconds (delayUs);
  }
...
void parseReceivedText()
{
  ...
  else if (textBuffer.substring(0,2) == "x+")
  {
    textBuffer = textBuffer.substring(2);
    steps = parseDigits();
    step (true, X_DIR, X_STP, 16*steps, ENX);
    printOK();
  }
  else if (textBuffer.substring(0,2) == "x-")
  {
    textBuffer = textBuffer.substring(2);
    steps = parseDigits();
    step (false, X_DIR, X_STP, 16*steps, ENX);
    printOK();
  }
  else if (textBuffer.substring(0,2) == "y+")
  {
    textBuffer = textBuffer.substring(2);
    steps = parseDigits();
    step (true, Y_DIR, Y_STP, 16*steps, ENY);
    printOK();
  }
  else if (textBuffer.substring(0,2) == "y-")
  {
    textBuffer = textBuffer.substring(2);
    steps = parseDigits();
    step (false, Y_DIR, Y_STP, 16*steps, ENY);
    printOK();
  }
  else if (textBuffer.substring(0,3) == "tcx")
  {
  ...
  else if (textBuffer.substring(0,3) == "tcy")
  {
  ...
  else if (textBuffer.substring(0,4) == "ttqx")
  {
    step (false, X_DIR, X_STP, 16*1000, ENX);
    step (true, X_DIR, X_STP, 16*1000, ENX);
    printOK();
  }
  else if (textBuffer.substring(0,3) == "ssp")
  ...
  else if (textBuffer.substring(0,3) == "pss")
  {//print 
    printStepSettings();
  }
  ...
  void printStepSettings()
  {
    response("HalfStep Microseconds: ");
    responseln(String(stepUs, DEC));
  }
  
  void readStepSettings()
  {
    stepUs = EEPROM.read(EEPROM_STEPUS_ADDR) * 4;
  }
  
  void doStepSettings()
  {
    if (textBuffer.substring(0,3) == "us=")  {
       textBuffer = textBuffer.substring(3);
       stepUs = parseDigits();
       printOK();
    }
  ...
  void writeStepSettings()
  {
    EEPROM.write(EEPROM_STEPUS_ADDR, stepUs/4);
  }
